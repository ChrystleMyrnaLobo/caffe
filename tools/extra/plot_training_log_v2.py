...
def get_log_parsing_script():
    ...
    return dirname + '/parse_log_v2.sh'
...
def create_field_index():
    ...
    # #Iters Seconds TrainLoss LearningRate TestLoss TestMAP TestAPCow TestAPDog TestARCow TestARDog
    field_index = { loss_key: {'Iters':0, 'Seconds':1
                             ,loss_key + ' ' + train_key :2
                             ,loss_key + ' learning rate':3
                             ,loss_key + ' ' + test_key:4 }
       , accuracy_key: {'Iters':0, 'Seconds':1
                             ,accuracy_key + ' mAP' + test_key:5
                             ,accuracy_key + ' APCow' + test_key:6 
                             ,accuracy_key + ' APDog' + test_key:7
                             ,accuracy_key + ' ARCow' + test_key:8
                             ,accuracy_key + ' ARDog' + test_key:9
       } }
    fields = set()
...
def get_data_file(chart_type, path_to_log):
    ...
    return (os.path.basename(path_to_log) + '.monitor')
...
def load_data(data_file, field_idx0, field_idx1):
    ...
    with open(data_file, 'r') as f:
        for line in f:
            ...
            if line[0] != '#':
                fields = line.split()
                #if int(fields[field_idx0].strip()) % 270 != 0:
                #    continue # Skip the 0th iteration
                if len(fields) > max(field_idx1, field_idx0):
